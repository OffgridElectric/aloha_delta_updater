defmodule Mix.Tasks.Aloha.Firmware.Patch do
  @shortdoc "Build an aloha firmware patch"
  ...
  *Note*: this implementation slightly differs from Nerves upstream
  `firmware.patch` script:
  * `rootfs.img` diff is built with xdelta3
  * other files are entirely copied if different from source, and not present if
    the same

  In particular, linux kernel image is not included when not updated.
  ...
  def run(args) do
    ...
    DeltaFile.create(source, target, output, work_dir)
